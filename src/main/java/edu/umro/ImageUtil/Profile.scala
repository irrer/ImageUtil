...
case class Profile(data: Seq[Float]) {
  ...
  val standardDeviation = ImageUtil.stdDev(data)
  ...
  val max: Double = {
    ...
    def approximate(xyList: IndexedSeq[XY], iteration: Int, prevX: Double): Double = {
      ...
      if ((iteration >= maxIteration) ||
        ...
        val newList = (0 to divisions).map(i => new XY(((i * incr) + lo), cubicSpline.evaluate((i * incr) + lo)))
        //Trace.trace("\n    " + newList.mkString("\n    "))
    ...
    val list = (0 to ((data.size - 1) * divisions)).map(i => new XY(i * incr, cubicSpline.evaluate(i * incr)))
