...
object ImageUtil {
  ...
  /** Least-squares fit to an arithmetic progression (even spacing).  Minimizes the positional
    * difference between input and output.
    *
    * Assumes points are in order and correspond to consecutive indices 0..n-1.
    * Returns the fitted evenly spaced points y_i = a + d*i.
    *
    * @author Generated by UM AI: GPT-5.2
  ...
  def evenlySpacedLeastSquares(pointList: Seq[Double]): Seq[Double] = {
    ...
    val n = pointList.length
    val ks = (0 until n).map(_.toDouble)

    val kMean = (n - 1) / 2.0
    val xMean = pointList.sum / n

    val sxx = ks.map(k => (k - kMean) * (k - kMean)).sum
    // If n == 1, spacing is undefined; return the single point.
    if (sxx == 0.0) {
      pointList
    } else {

      val sxy = ks.zip(pointList).map { case (k, x) => (k - kMean) * (x - xMean) }.sum

      val d = sxy / sxx
      val a = xMean - d * kMean

      val result = ks.map(k => a + d * k)
      result
    }
  }
