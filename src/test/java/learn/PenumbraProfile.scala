  ...
  def dumpFile(file: File) {
    try {
      ...
      if (al.isImage) {
        ...
        badPixelList.map(w => ImageUtil.annotatePixel(bufImage, w.x, w.y, Color.YELLOW, w.x + ", " + w.y))
        val imageFileName = file.getName.toLowerCase.replaceAll(".dcm$", "_" + badPixelList.size + ".png")
