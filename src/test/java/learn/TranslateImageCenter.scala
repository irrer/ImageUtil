...
object TranslateImageCenter {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (false) {
      ...
      addMult(TagFromName.XRayImageReceptorTranslation, Seq(2.0, 3.0, 500.0))
      add(TagFromName.Rows, imageSize.toString)
      add(TagFromName.Columns, imageSize.toString)
      add(TagFromName.RTImageSID, "1500.0")
      add(TagFromName.RadiationMachineSAD, "1000.0")
      addMult(TagFromName.ImagePlanePixelSpacing, Seq(pixSize, pixSize))
      val o = ((imageSize / 2) * pixSize) - (pixSize / 2)
      addMult(TagFromName.RTImagePosition, Seq(-o, o))
    } else {
      val real = new AttributeList
      real.read(new File("""D:\tmp\0001.dcm"""))
      def cpy(tag: AttributeTag) = al.put(real.get(tag))
      cpy(TagFromName.XRayImageReceptorTranslation)
      cpy(TagFromName.Rows)
      cpy(TagFromName.Columns)
      cpy(TagFromName.RTImageSID)
      cpy(TagFromName.RadiationMachineSAD)
      cpy(TagFromName.ImagePlanePixelSpacing)
      cpy(TagFromName.RTImagePosition)
    }
